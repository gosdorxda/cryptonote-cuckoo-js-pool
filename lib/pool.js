...
if (config.blockNotifier.port)
{
	var ctrl_server = net.createServer(function (localsocket) {
		...
		jobRefresh('get_template','notify');
	});
...
function BlockTemplate(template){
	...
	this.hashblob = template.blockhashing_blob.slice(0,-16);
	...
	Buffer.from(template.blocktemplate_blob, 'hex').copy(this.previous_hash,0,7,39);
}
BlockTemplate.prototype = {
	nextBlob: function(){
		var jobnoncebuffer = Buffer.allocUnsafe(4);
		jobnoncebuffer.writeUInt32BE(++this.extraNonce,0);
			
		return this.hashblob+instanceId.toString('hex')+jobnoncebuffer.toString('hex');;
	}
};
...
function getBlockTemplate(callback){
	apiInterfaces.rpcDaemon('getblocktemplate', {reserve_size: 0, wallet_address: config.poolServer.poolAddress}, callback);
}
...
function jobRefresh(state,trigger){
	...
	case "check_count":
		getBlockCount(function(error, result){
			if (error){
				...
				if(currentBlockTemplate) currentBlockTemplate.valid=false;
				return;
			}

			if(result.count == currentBlockHeight) {
				jobRefresh("check_hash",trigger);
				return;
			}

			log('info', logSystem, 'Blockchain height changed to %d, updating template. (%s)', [currentBlockHeight,trigger]);
			jobRefresh("get_template",trigger);
			return;
		});
	break;
	...
	case "check_hash":
		getBlockHash(function(error, result){
			...
			if((result == currentBlockHash)&&(currentBlockTemplate.valid == true)) {
				if(jobRefreshCompleteCallback != null)
					jobRefreshCompleteCallback(true);
				return;
			}

			log('info', logSystem, 'Blockchain hash changed to %s, updating template.(%s)', [currentBlockHash,trigger]);
			jobRefresh("get_template",trigger);
			return;
		});
	break;
	...
	case "get_template":
		getBlockTemplate(function(error, result){
			...
			if (!currentBlockTemplate || currentBlockTemplate.valid == false || previous_hash.toString('hex') != currentBlockTemplate.previous_hash.toString('hex')){
				for (var minerId in connectedMiners){
					var miner = connectedMiners[minerId];
					miner.job = null;
				}
				log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d (%s)', [result.height, result.difficulty,trigger]);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage, setMinerid){
	...
	switch(method){
		...
		case 'submit':
			...
			if (!miner.job || miner.job.id != params.job_id.toString() || currentBlockTemplate.valid == false){
				sendReply({code: -32503, message: "stale"}, null, 'submit');
				var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
				log('info', logSystem, 'stale share: ' + JSON.stringify(params) + ' from ' + minerText);
				return;
			}
...
function processShare(miner, params){
	...
	var cycle = Buffer.allocUnsafe(cycleLength*4);
	for(var i=0;i<cycleLength;i++)
	{
		cycle.writeUInt32LE(params.pow[i], i*4);
	}
	var noncebuffer = Buffer.allocUnsafe(4);
	noncebuffer.writeUInt32BE(params.nonce,0);
	
	var jobnoncebuffer = Buffer.allocUnsafe(4);
	jobnoncebuffer.writeUInt32BE(miner.job.extraNonce,0);
			
	var header = Buffer.concat([Buffer.from(currentBlockTemplate.hashblob, 'hex'),instanceId,jobnoncebuffer,noncebuffer]);
																				
	var prooferror = cuHashing.c29b(header,params.pow);
	...
	if (s_difficulty >= b_difficulty){
		...
		var shareBuffer = Buffer.from(currentBlockTemplate.blob, 'hex');

		instanceId.copy(shareBuffer,39,0,4);
		jobnoncebuffer.copy(shareBuffer,43,0,4);
		shareBuffer.writeUInt32LE(params.nonce,47);
		
		for(var i=0;i<cycleLength;i++)
		{
			shareBuffer.writeUInt32LE(params.pow[i], 51+(i*4));
		}
		...
		apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
			if (error){
				...
				log('info', logSystem,
					...
					[result.hash.substr(0, 6), height, miner.login, miner.ip, result]
				);
				if(currentBlockTemplate.height == height) currentBlockTemplate.valid=false;
				jobRefresh('get_template','block found');
				recordShareData(miner, height, j_difficulty, j_score , s_difficulty, true, result.hash, b_difficulty);
			}
		});
	}
